...
public final class PatientResourceProviderIT {
	...
	/**
	 * Verifies that the correct bene id is returned when a hicn points to more
	 * than one bene id in either the Beneficiaries and/or BeneficiariesHistory
	 * table.
	 */
	@Test
	public void searchForExistingPatientByHicnHashWithBeneDups() {
		...
		loadedRecords.addAll(
				ServerTestUtils.loadData(Arrays.asList(StaticRifResourceGroup.SAMPLE_HICN_MULT_BENES.getResources())));

		IGenericClient fhirClient = ServerTestUtils.createFhirClient();

		Stream<Beneficiary> beneficiariesStream = loadedRecords.stream()
				.filter(r -> r instanceof Beneficiary).map(r -> (Beneficiary) r);
		List<Beneficiary> beneficiariesList = beneficiariesStream.collect(Collectors.toList());

		Stream<BeneficiaryHistory> beneficiariesHistoryStream = loadedRecords.stream()
		...
		assertPatientByHicnHashWithBeneDupsMatch(fhirClient, beneficiariesList, beneficiariesHistoryList, "567834",
				"543217066U", useHicnFromBeneficiaryTable);
		 ...
		 */
		useHicnFromBeneficiaryTable = true;
		assertPatientByHicnHashWithBeneDupsMatch(fhirClient, beneficiariesList, beneficiariesHistoryList,
				"123456NULLREFYR", "543217066N", useHicnFromBeneficiaryTable);
		 ...
		 */
		useHicnFromBeneficiaryTable = true;
		assertPatientByHicnHashWithBeneDupsMatch(fhirClient, beneficiariesList, beneficiariesHistoryList, "BENE1234",
				"SAMEHICN", useHicnFromBeneficiaryTable);
		...
		assertPatientByHicnHashWithBeneDupsMatch(fhirClient, beneficiariesList, beneficiariesHistoryList, "55555",
				"HISTHICN", useHicnFromBeneficiaryTable);
		 ...
		 */
		useHicnFromBeneficiaryTable = false;
		assertPatientByHicnHashWithBeneDupsMatch(fhirClient, beneficiariesList, beneficiariesHistoryList, "66666",
				"DUPHISTHIC", useHicnFromBeneficiaryTable);
		...
		Bundle searchResults = fhirClient.search().forResource(Patient.class)
				.where(Patient.IDENTIFIER.exactly()
						.systemAndIdentifier(TransformerConstants.CODING_BBAPI_BENE_HICN_HASH, "notfoundhicn"))
				.returnBundle(Bundle.class).execute();
		Assert.assertEquals(0, searchResults.getTotal());
	...
	/**
	 * The following method tests which beneficiary record should be returned
	 * when there are instances of one hicn pointing to more than bene id
	 * between the Beneficiaries and BeneficiariesHistory tables.
	 * 
	 * @param fhirClient
	...
	private void assertPatientByHicnHashWithBeneDupsMatch(IGenericClient fhirClient,
			List<Beneficiary> beneficiariesList, List<BeneficiaryHistory> beneficiariesHistoryList,
			String beneficiaryId, String hicnUnhashed, Boolean useHicnFromBeneficiaryTable) {

		String hicnHashed;
		if (useHicnFromBeneficiaryTable) {
			Beneficiary beneficiaryHicnToMatchTo = beneficiariesList.stream()
					.filter(r -> hicnUnhashed.equals(r.getHicnUnhashed().get())).findFirst().get();
			hicnHashed = beneficiaryHicnToMatchTo.getHicn();
		} else {
			BeneficiaryHistory beneficiaryHistoryHicnToMatchTo = beneficiariesHistoryList.stream()
					.filter(r -> hicnUnhashed.equals(r.getHicnUnhashed().get())).findFirst().get();
		...
		Bundle searchResults = fhirClient.search().forResource(Patient.class)
				.where(Patient.IDENTIFIER.exactly()
						.systemAndIdentifier(TransformerConstants.CODING_BBAPI_BENE_HICN_HASH,
								hicnHashed))
				.returnBundle(Bundle.class).execute();
		...
		Beneficiary beneficiary = beneficiariesList.stream().filter(r -> beneficiaryId.equals(r.getBeneficiaryId()))
				.findAny().get();
		Patient patientFromSearchResult = (Patient) searchResults.getEntry().get(0).getResource();
		BeneficiaryTransformerTest.assertMatches(beneficiary, patientFromSearchResult);
	}
